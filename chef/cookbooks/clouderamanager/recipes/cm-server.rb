...
=end
        ...
        cluster_config.each do |r|
          if r[:role_type] == 'NAMENODE'
            ...
            # <ApiCommand> (name:HdfsStartWithFailovers, id:58, active:false, success:false)
            id = cmd[0].getattr('id')
            active = cmd[0].getattr('active')
            success = cmd[0].getattr('success')
            if success == 'true'
              Chef::Log.info("CM - HDFS formatted sucessful [#{id}, #{active}]") if debug
            else
              Chef::Log.info("CM - HDFS already formatted - skipped [#{id}, #{active}]") if debug
            end
          end
        end
        ...
        # Start HDFS. 
        #--------------------------------------------------------------------
        Chef::Log.info("CM - starting HDFS service") if debug
        cmd = api.start_service(hdfs_service)
        # if not cmd.wait(cmd_timeout).success
        #   Chef::Log.info("CM - HDFS start failed")
        # end        
        Chef::Log.info("CM - HDFS service start results #{cmd}") if debug
        # <ApiCommand> (name:HdfsStartWithFailovers, id:63, active:false, success:false)
        id = cmd.getattr('id')
        active = cmd.getattr('active')
        success = cmd.getattr('success')
        if success == 'true'
          Chef::Log.info("CM - HDFS service startup sucessful [#{id}, #{active}]") if debug
        else
          Chef::Log.info("CM - HDFS service already running - skipped [#{id}, #{active}]") if debug
        end
      end
    end
  end
