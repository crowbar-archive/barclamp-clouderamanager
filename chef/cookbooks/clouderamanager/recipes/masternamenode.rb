...
Chef::Log.info("CLOUDERAMANAGER : BEGIN clouderamanager:masternamenode") if debug
...
if node[:clouderamanager][:cluster][:valid_config]
  ...
  Chef::Log.info("CLOUDERAMANAGER : CONFIGURATION VALID - STARTING MASTER NAME NODE SERVICES")
  ...
  if (!File.exists?("#{hb}/meta1/current/VERSION")) 
    ...
    Chef::Log.info("CLOUDERAMANAGER : RUNNING HDFS FORMAT NOW") if debug
...
EOH
    ...
    Chef::Log.info("CLOUDERAMANAGER : HDFS ALREADY FORMATTED") if debug
  end
  ...
  Chef::Log.info("CLOUDERAMANAGER : CONFIGURATION INVALID - STOPPING MASTER NAME NODE SERVICES")
...
#######################################################################
Chef::Log.info("CLOUDERAMANAGER : END clouderamanager:masternamenode") if debug
